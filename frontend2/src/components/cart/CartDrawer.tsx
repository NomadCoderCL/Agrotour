      ...
      <div className="p-4 overflow-auto h-[calc(100%-160px)]">
        {carro.length === 0 ? (
          ...
          <div className="space-y-4">
            {carro.map((item) => (
              <div key={String(item.producto_id || item.id || item.nombre)} className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <button onClick={() => eliminarDelCarro(item.producto_id)} className="text-sm text-red-600">Eliminar</button>
                </div>
              </div>
            ))}
          </div>
