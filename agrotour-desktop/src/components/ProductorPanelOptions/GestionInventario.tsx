...
const GestionInventario: React.FC = () => {
  // ─── Estado (preparado para reemplazar con useQuery) ─────────────
  const [productos, setProductos] = useState<Producto[]>(DEMO_PRODUCTOS);
  const [buscar, setBuscar] = useState('');
  const [modalOpen, setModalOpen] = useState(false);
  const [editando, setEditando] = useState<Producto | null>(null);
  const [nextId, setNextId] = useState(DEMO_PRODUCTOS.length + 1);
  ...
  // ─── CRUD ────────────────────────────────────────────────────────
  const onSubmit = useCallback((data: ProductoFormData) => {
    if (editando) {
      // Actualizar
      setProductos((prev) =>
        prev.map((p) => p.id === editando.id ? { ...p, ...data, precio: Number(data.precio), stock: Number(data.stock) } : p)
      );
    } else {
      // Crear
