...
class ApiClient {
  ...
  constructor(baseURL?: string, retryConfig?: Partial<RetryConfig>) {
    const API_BASE_URL = baseURL || import.meta.env.REACT_APP_API_URL || "http://localhost:8000/api";
  ...
  async getProducto(id: number) {
    return this.retryWithBackoff(() =>
      this.axiosInstance.get(`/productos/${id}/`).then((res) => res.data)
    );
  }

  async createProducto(data: any) {
    return this.retryWithBackoff(() =>
      this.axiosInstance.post("/productos/", data).then((res) => res.data)
    );
  }
  ...
  async deleteProducto(id: number) {
    return this.retryWithBackoff(() =>
      this.axiosInstance.delete(`/productos/${id}/`).then(() => ({}))
    );
  }
  ...
  async getUbicaciones() {
    return this.retryWithBackoff(() =>
      this.axiosInstance.get("/ubicaciones/").then((res) => res.data)
    );
  }
  ...
  async getVentas() {
    return this.retryWithBackoff(() =>
      this.axiosInstance.get("/ventas/").then((res) => res.data)
    );
  }
  ...
  async getBoletas() {
    return this.retryWithBackoff(() =>
      this.axiosInstance.get("/boletas/").then((res) => res.data)
    );
  }
  ...
  async getVisitas() {
    return this.retryWithBackoff(() =>
      this.axiosInstance.get("/visitas/").then((res) => res.data)
    );
  }

  async crearVisita(data: any) {
    return this.retryWithBackoff(() =>
      this.axiosInstance.post("/visitas/", data).then((res) => res.data)
    );
  }

  async actualizarVisita(id: number, data: any) {
    return this.retryWithBackoff(() =>
      this.axiosInstance.put(`/visitas/${id}/`, data).then((res) => res.data)
    );
  }
