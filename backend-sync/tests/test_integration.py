...
# We assume the docker container is running
...
def client_fixture():
    ...
    # TEARDOWN: Already handled by next test's cleanup
...
def test_rls_integration_middleware(client):
    ...
    # Seed Products for foreign key constraints
    from app.models import Product
    
    with Session(engine) as session:
        # Insert for Tenant A
        session.exec(text(f"SET app.current_tenant_id = '{tenant_a}';"))
        sku_a = f"SKU_A_{uuid4().hex[:8]}"  # Unique SKU per run
        prod_a = Product(
            id=product_id_a, 
            tenant_id=tenant_a, 
            name="Prod A", 
            price=10.0, 
            sku=sku_a,  # Use unique SKU
            current_stock=100, 
            created_by=uuid4(), 
            updated_by=uuid4(), 
            device_id=uuid4(), 
        ...
        sku_b = f"SKU_B_{uuid4().hex[:8]}"  # Unique SKU per run
        prod_b = Product(
            id=product_id_b, 
            tenant_id=tenant_b, 
            name="Prod B", 
            price=20.0, 
            sku=sku_b,  # Use unique SKU
            current_stock=50, 
            created_by=uuid4(), 
            updated_by=uuid4(), 
            device_id=uuid4(), 
    ...
    assert response.status_code == 200
    print("✅ Tenant B Push Success")

    # 3. Pull data as Tenant A - Should NOT see Tenant B data
    response = client.post(
        "/sync/pull",
        json={
            "tenant_id": tenant_a,
    ...
    ops = data["operations"]
    print(f"✅ Tenant A Pull: Received {len(ops)} operations")
    
    assert len(ops) >= 1
    # Verify all ops belong to tenant_a
    for op in ops:
        assert op["tenant_id"] == tenant_a
        assert op["product_id"] == product_id_a
    
    print("✅ RLS Enforcement Verified: Tenant A saw only Tenant A data")
    ...
    print(f"✅ Tenant B Pull: Received {len(ops_b)} operations")
...
def test_middleware_missing_header(client):
    ...
    print("✅ Middleware Validation Verified")


if __name__ == "__main__":
    # Create client manually to run test without pytest runner if needed
    t = TestClient(app)
    # Run setup manually
    create_db_and_tables()
    # Run test function
    try:
        test_rls_integration_middleware(t)
